...
class ScaledDotProductAttentionForMemory(nn.Module):
    """
    Scaled Dot-Product Attention proposed in "Attention Is All You Need"
    Compute the dot products of the query with all keys, divide each by sqrt(dim),
    and apply a softmax function to obtain the weights on the values
    ...
    """
    def __init__(self, in_features, out_features):
        super(ScaledDotProductAttentionForMemory, self).__init__()
        self.q_proj = nn.Linear(in_features, out_features)
        self.k_proj = nn.Linear(in_features, out_features)
    ...
    def forward(self, 
        ...
        d_model = q_out.size()[-1]
        attn_logits = torch.matmul(q_out, k_out.T) / np.sqrt(d_model)
    
        attn_weights = F.softmax(attn_logits/temperature, -1)
